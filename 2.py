...
def solve_pure_exchange_battery_with_coords(N=6, coords=None, t_max=15.0):
    ...
    # --- 改进后的初态设定：前一半激发 (e)，后一半基态 (g) ---
    half_N = N // 2
    # 前一半设为 basis(2, 0) [激发态]，后一半设为 basis(2, 1) [基态]
    ...
    plt.tight_layout()
    plt.savefig('Fig_Star_Topology_DynamicsPPP.pdf', format='pdf', bbox_inches='tight')
    plt.show()


# ==========================================
# ⚡ 调用示例：N=7 的 Ergotropy 冠军坐标
# ==========================================
best_x_star = [0.   ,  0.4358 ,0.4458 ,0.4649 ,1.7653 ,1.7753 ,1.7876 ,1.8686]
solve_pure_exchange_battery_with_coords(N=8, coords=best_x_star)