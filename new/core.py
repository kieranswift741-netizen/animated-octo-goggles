...
def calc_ergo(rho_or_psi, targets, H_B_tgt, E_n_asc):
    """
    计算可提取功（Ergotropy）
    
    参数:
        rho_or_psi: 量子态（密度矩阵或波函数）
        targets: 目标电池的原子索引
        H_B_tgt: 目标电池子系统的哈密顿量
        E_n_asc: H_B_tgt的本征能量（升序）
...
def scan_topology_space(N=6, num_samples=30, t_max=15.0, excitation_mode='alternating'):
    """
    执行完整的拓扑相空间扫描
    ...
    for idx, x_inner in enumerate(configs):
        ...
        # 添加可提取功计算
        def ergo_callback(t, rho_or_psi):
            return calc_ergo(rho_or_psi, targets, H_B_tgt, E_n_asc)
        e_ops.append(ergo_callback)
    ...
    plt.savefig(f'Fig_Full_Topology_Scan_N{N}.pdf', format='pdf', bbox_inches='tight')
    # plt.show()

    return {
        'best_P_config': best_P_x_full,
        'best_E_config': best_E_x_full,
        'results_chi': results_chi,
        'results_P_avg': results_P_avg,
        'results_Ergo': results_Ergo
    }
